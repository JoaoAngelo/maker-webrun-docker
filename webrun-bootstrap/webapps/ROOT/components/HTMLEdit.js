...
HTMLEdit.prototype.designComponent = function (doc) {
  ...
  if (this.type == 2) { // Input de data (calendário)
    // �? necessário criar outro div para permitir a colocação do botão
  ...
  if (this.type == 2) { // Input de data (calendário)
    ...
    // Alterar o layout do input para suportar o botão de calendário
    ...
    if (this.mobile) {
      ...
      this.input.addEventListener("focusin", function() {
        if (window.dtpComponent && window.dtpComponent.dtpInitialized) {
          ...
          // �? necessário focar o input novamente.
          setTimeout(function() {
            object.input.focus();
          }, 100);
        }
