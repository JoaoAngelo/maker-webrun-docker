...
function HTMLGrid(sys, formID, code, posX, posY, width, height, formCode) {
  ...
  this.toolbarHeight = 40; //altura da barra de navegação / top do corpo
  this.headerHeight = 38; //altura padrao do cabecalho
  this.cellHeight = 33; //altura padrao da linha
...
HTMLGrid.prototype.designComponent = function() {
  ...
  // Cria barra de navegação do componente.
  ...
  // Cria barra de paginação na navegação da grade.
  ...
  /**
   * @author Janpier
   * Essa funcionalidade sobrescreve o valor padrão da grade, assim a function é executada retornando o quantidade de regristros geral e de cada grupo.
   */
    ...
    filterData: function() {
      ...
      if((!grid.editing && !grid.inserting)){
        ...
        if (grid && !grid.filterMode || grid.filterMode === 0) {
          // Verifica se existe filto preenchido, tanto avançado quanto normal.
  ...
  /**Sobrescreve o método de redraw para Internet Explorer*/
  ...
  //anexa navegação à div principal do componente
  this.div.appendChild(this.bar);
  this.div.appendChild(pC);
  /**Cancela a seleção de linha com o clique direito do mouse. */
  this.iscCanvas.body.canSelectOnRightMouse = false;

  //executa métodos da navegação
...
HTMLGrid.prototype.getSelectedRow = function (keyPress) {
  ...
  /*Quando está agrupado a busca irá ser realizado  ou no original data ou nos items do agrupamento.*/
...
/**
 * Método responsável por obter o real index da linha selecionada, ignorando o estado da grade.
 * @author Janpier
 * @returns linha real;
 */
...
HTMLGrid.prototype.setColumns = function (cols, width, components) {
  ...
  for (var i = 0; i < components.length; i++) {
    ...
    if (field && field.summaryFunction && presetArray.includes(field.summaryFunction) && cols[i].type == "double") {
  ...
  if (!objectIsEmpty(summaryFields)) {
    var startGroup = [];
    var startGroupField = summaryFields.filter(function (elem) {
      return elem['startGroup'] === true;
    });
    if (startGroupField) {
      startGroupField.map(function (group) {
        startGroup.push(group.name);
      });
    }
...
/**
 * Método responsável pro ajustar as colunas quando o formulário é responsivo, esse método só é chamado na criação do componente.
 * @author Janpier
...
HTMLGrid.prototype.setReadOnly = function(v) {
  ...
  // Manter a variável this.readOnly para as funções da API.
  this.readOnly = this.readonly;

  // Definir a propriedade somente leitura na barra de navegação da grade.
  ...
  if (this.iscCanvas.getEditRow() !== row) {
    ...
    if (flowCondition.length > 0) {
      ...
      for (var i = 0; i < flowCondition.length; i++) {
        if (grouped) {
          paint = (flowCondition[i].row === record.row);
        } else {
...
/**
 * @param notRefresh - parâmetros que informa se deverá ser realizado refresh do data source.
 */
...
/**Método que salva as configurações aplicada pelo usuário no editor de filtro
 * @author Janpier
...
/**
 * Método responsável pelo parse do critério a serem enviados para o servidor.
 * @author Janpier.
...
/**Método responsável por checar se existe somente criterios informado no filtro.
 * @author Janpier
 * @criteira filtro aplicado
 * @returns onlyCriteria valor lógico informando se o filtro só possui criterios
*/
...
/**
 * Método responsável por realizar o parse do valor informado pelo usuário
 * @author Janpier.
 * @param cod código do componente
...
/**Método responsável por validar os valores inseridos para filtro de acordo o tipo do dado
 * @author Janpier
 * @criteira filtro aplicado
 * @returns onlyCriteria valor lógico informando de o filtro é válido.
*/
...
/** Método responsável por checar o valor de acordo o tipo de dado da coluna
 * @author Janpier
 * @param type tipo da coluna.
 * @param value valor da coluna.
 * @param col coluna.
 * @returns validated valor lógico.
*/
...
HTMLGrid.prototype.openNormalForm = function (include) {
  ...
  let jsonPorperties = {};
  jsonPorperties.formGUID = mainform.formGUID;
  jsonPorperties.component = this.id;
  jsonPorperties.method = 'refreshPage';
  jsonPorperties.action = 'wea';
  var otherProperties = (", gotoRow: " + gotoRow + ", onClose: true, jsonProperties:'" + JSON.stringify(jsonPorperties) + "'");
...
HTMLGrid.prototype.completePost = function (cancel, newDataRow, newKeyData, newQueryValues) {
  try {
    this.navigationEndEditing();
    var ref = this.iscCanvas;
    /* Quando a grade está agrupada a linha que está sendo editada/inserida difere da linha real,
    isso ocorre devido cada linha do grupo/sumário ser considerados.
    **/
    ...
    if (this.editing || this.inserting) {
      ...
      while (sizeFields > colNum) {
        ...
        if (elem) {
          /**(Compatibilização com a grades legadas) -> Obtém o valor do componente não vinculado DB em tela, atualizando o data*/
...
HTMLGrid.prototype.gridKeyPress = function (evt) {
  ...
  else if (!altKey && ctrlKey && keyCode == 39) { //�?ltima página
...
HTMLGrid.prototype.checkDoubleClick = function (ref) {
  if (!ref.editing && !ref.inserting) {
    ...
    if (!ref.ondblclick || !ref.doubleClickOnly) {
      if (ref.editable && (!ref.lockEditable)) {
...
/**
 * Método responsável por reordenar uma coluna.
 * @author Janpier
 * @param old posição antiga.
...
/**
 * Método responsável por montar o JSON que salva as costumizações do usuário.
 * @author Janpier
...
/**
 * Método responsável por mostrar ou ocultar uma coluna.
 * @author Janpier
 * @param num número da coluna
 * @param value 1 ou 0 para mostrar ou não a coluna.
 * @param comp componente responsável pela coluna no formulário
 */
...
/**
 * Método responsável por fazer com que a grade volte ao estado inicial.
 * @author Janpier
 */
...
/**
 * Essa função retorna o real index de um linha na grade dependendo do seu estado atual.
 * @author Janpier
 * @param record linha que está selecionada.
...
HTMLGrid.prototype.gridResizeResponsive = function() {
  ...
  if (tabLiElem) {
    ...
    if (tabAElem && tabAElem.length > 0) {
      // Associar evento de clique ao botão da aba do componente Aba.
...
/**
 * Método responsável por realizar o resize das colunas da grade.
 * @author Janpier.
 * @param resizeGrid percentual para resize.
 */
HTMLGrid.prototype.resizeColumns = function(resizeGrid) {
  if (!this.userCustomColumnWidth) {
    if (this.tab && !this.tab.selected) return;
...
/**
 * Método responsável por realizar o ajuste do botão de filtro
 * @author Marcos.
 */
...
function gridResize(grid) {
  if (grid && getWindowDimensions().width !== 0) {
    let newWidth = 0;
    // Se a grade não está visível realiza a o cálculo pela porcentagem.
    if (!grid.visible) newWidth = getWindowDimensions().width * (parseFloat(grid.div.style.width) / 100);
    else newWidth = grid.div.offsetWidth > 0 ? grid.div.offsetWidth : grid.width;

    //Se a grade não sofreu resize não é necessário redimensionar.
...
/**
 * Método responsável por realizar a verificação das colunas a serem pintadas quando a grade possuir colunas ocultas.
 * @author Janpier
 * @param grid
 * @param colsPaint
...
/**
 * Método responsável por renderizar o filtro avançado da grade.
 * @author Janpier
 * @param grid
 */
function gridAdvancedFilter(grid) {
  ...
  if (grid) {
    if (!grid.filterModal) {
      ...
      // Obter conteúdo do modal para limitar a altura.
      if (IE) {
