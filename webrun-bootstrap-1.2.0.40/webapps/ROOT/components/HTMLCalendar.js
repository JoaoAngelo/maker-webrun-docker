...
HTMLCalendar.prototype.updateData = function() {
  ...
  var y = this.calendar.view.currentStart.getFullYear(), m = this.calendar.view.currentStart.getMonth();
  var firstDay = new Date(y, m, 1, 0, 0, 0).format("yyyy-MM-dd HH:mm:ss");
  var lastDay = new Date(y, m + 1, 0, 23, 59, 59).format("yyyy-MM-dd HH:mm:ss");
    ...
    function(response) {
      ...
      if (response && response.events) {
        for (var i = 0; i < response.events.length; i++) {
          ...
          object.addEvent(event.title, event.start, event.end, event.id, false);
        }
      }
      ...
      if (response && response.holidays) {
        for (var i = 0; i < response.holidays.length; i++) {
          ...
          var holidayDate = moment(holiday.date).local();
...
HTMLCalendar.prototype.selectDateAction = function(e) {
  ...
  // Chamar evento "Ao Clicar no Dia"
  if (this.AoClicarNoDia) {
    this.AoClicarNoDia.call(this, e.dateStr);
  }
 ...
 **/
HTMLCalendar.prototype.addEvent = function(title, start, end, id, save) {
  var object = this;
  var eventStart = moment(start).local();
  var eventEnd = end ? moment(end).local() : null;
  ...
  if (save && this.editable) {
    ...
    this.sendRequest("a", null, {
      ...
      start: moment(start).utc().toISOString(),
        ...
        moment(start).utc().toISOString() :
        moment(end).utc().toISOString(),
      allDay: allDay
    }, function(response) {
      // Verificar a resposta do servidor.
...
HTMLCalendar.prototype.createEventModal = function(type, startDate, endDate, allDay, event) {
  ...
  if (this.hasEditForm()) {
    ...
    if (type == 0) { // Adicionar Evento
      editForm = getAndEvalSync2(getAbsolutContextPath() + "componentData.do?action=componentData&sys=" + URLEncode(this.sys, 'GET') + "&formID=" + URLEncode(this.formID, 'GET') + "&comID=" + URLEncode(this.code, 'GET') + "&type=a");
    } else if (type == 1) { // Editar evento
      editForm = getAndEvalSync2(getAbsolutContextPath() + "componentData.do?action=componentData&sys=" + URLEncode(this.sys, 'GET') + "&formID=" + URLEncode(this.formID, 'GET') + "&comID=" + URLEncode(this.code, 'GET') + "&req=" + URLEncode(event.id, 'GET') + "&type=e");
    }

    try {
      if (editForm) {
        // Verificar se o formulário é uma janela separada.
        if (isWindow(editForm)) {
          // Associar evento onbeforeunload a janela.
          editForm.addEventListener("beforeunload", function() {
            if (object) object.updateData();
          }, false);

        // Verificar se o formulário é uma janela flutuante.
        } else if (editForm.isFloatingForm === true) {
          // Associar evento onbeforeclose ao formulário flutuante.
          editForm.onbeforeclose = function() {
            if (object) object.updateData();
          };
        }
      }
