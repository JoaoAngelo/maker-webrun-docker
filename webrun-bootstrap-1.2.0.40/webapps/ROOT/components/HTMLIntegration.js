...
HTMLIntegration.prototype.designComponent = function(doc) {
  ...
  if (this.Iframe && this.Iframe.length && this.Iframe.length > 0 && this.Iframe.toLowerCase() === "true") {
    ...
    this.pageFrame.src = getAbsolutContextPath() + "tmp/" + this.sys + "/" + this.formID + "/" + this.code + ".html" +
...
HTMLIntegration.prototype.assignEvents = function(events) {
  if (events && events.length > 0) {
    for (var i = 0; i < events.length; i++) {
      var event = events[i];
      var element = document.getElementById(event.element);
      if (element) element.addEventListener(event.event, function() {
        if (event.params && event.params.length && event.params.length > 0) {
          ebfFlowExecute(event.flow, event.params);
        } else {
          ebfFlowExecute(event.flow);
        }
      })
    }
  }
