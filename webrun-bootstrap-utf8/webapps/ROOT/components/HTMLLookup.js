...
HTMLLookup.prototype.setShowValue = function(value) {
  ...
  if (this.mobileInput && (!this.mobileInput.options || this.mobileInput.options.length == 0)) {
    ...
    if (this.mobileInput && this.mobileInput.options
      && this.mobileInput.options.length === 1) {
      this.mobileInput.options[0].innerHTML = replaceAll(replaceAll(value, '>', '&gt;'), '<', '&lt;');
    }
  }
};
  ...
  if (this.mobile) {
    ...
    if (this.lookup) {
      ...
      if (xmldoc != null) {
        ...
        // Verificar se permite inserir, se nÃ£o possui opÃ§Ãµes e se possui texto.
        if (this.allowInsert && this.lookupDataLength <= 1 &&
          ...
          // Criar opÃ§Ã£o pra adicionar.
          this.lookupAddOption = document.createElement("a");
          this.lookupAddOption.href = "#";
          this.lookupAddOption.className = "list-item-action w-100 px-2 py-1"; // Bootstrap
          ...
          // Associar evento de clique a opÃ§Ã£o de adicionar.
...
HTMLLookup.prototype.lookupSelectPress = function(evt) {
  ...
  if (evt.key == "PageUp" || keyCode == 33) {
    ...
    if (x < 0) x = -1;
    this.lookupInput.selectedIndex = x;
    r = false;
  } else if (evt.key == "PageDown" || keyCode == 34) {
    var x = this.lookupInput.selectedIndex + 10;
    if (x > this.lookupInput.length - 1) x = this.lookupInput.length - 1;
    this.lookupInput.selectedIndex = x;
    r = false;
  } else if ((evt.key == "Escape" || keyCode == 27) ||
             (evt.key == "Tab" || keyCode == 9)) {
    ...
    this.lookupChangeSelectValue(evt, this.lookupInput);
    r = false;
  } else if ((evt.key == "ArrowUp" || keyCode == 38) && this.lookupInput.selectedIndex == 0) {
    this.lookupInput.options[0].selected = false;
  } else if (!evt.altKey && !evt.ctrlKey && ((evt.key == "ArrowRight" || keyCode == 39) ||
                                             (evt.key == "ArrowLeft" || keyCode == 37))) {
    ...
    this.lookupInput.selectedIndex = idx;
    this.lookupInput.focus();
  }
...
/**
 * Verifica se o valor informado por parÃ¢metro Ã© nulo ou vazio.
 * @param value
 * @returns valor lÃ³gico.
 */
