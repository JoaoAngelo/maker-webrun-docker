(function (global, factory) {
	...
	*/
	...
	/*
	 * function: èŽ·å�–æµ�è§ˆå™¨åœ°å�€æ �ä¸­çš„æŒ‡å®šå�‚æ•°.
	 * key: å�‚æ•°å��
	 * url: é»˜è®¤ä¸ºå½“å‰�åœ°å�€æ �
	 */
	...
	function _GET(key) {
	  ...
	  /*
	   * æ³¨æ„�è¿™é‡Œæ­£åˆ™è¡¨è¾¾å¼�çš„ä¹¦å†™æ–¹æ³•
	   * (^|&)keyåŒ¹é…�: ç›´æŽ¥ä»¥keyå¼€å§‹æˆ–ä»¥&keyå¼€å§‹çš„å­—ç¬¦ä¸²
	   * å�Œç�†(&|$)è¡¨ç¤ºä»¥&ç»“æ�Ÿæˆ–æ˜¯ç›´æŽ¥ç»“æ�Ÿçš„å­—ç¬¦ä¸²
	   * ...å½“ç„¶, æˆ‘å¹¶ä¸�çŸ¥é�“è¿™ç§�ç”¨æ³•.
	   */
	...
	/*
	 * function: æ ¹æ�®ç»™å®šå�‚æ•°ç”Ÿæˆ�urlåœ°å�€
	 * var dic = {name: 'genreal', age: 24}
	 * var url = 'https://www.baidu.com?age=22';
	 * _buildUrl(dic, url);
	 * å°†å¾—åˆ°"https://www.baidu.com?age=24&name=genreal"
	 * å“¦, å¿½ç•¥å…ˆå�Žé¡ºåº�å�§...
	 *
	 * è¡¥å……: å�¯ä»¥å�‚è€ƒæµ�è§ˆå™¨URLSearchParamså¯¹è±¡, æ›´åŠ æ–¹ä¾¿å’Œå¼ºå¤§.
	 * è€ƒè™‘åˆ°å…¼å®¹æ€§, æš‚æ—¶ä¸�ä½¿ç”¨è¿™ä¸ªå·¥å…·.
	 */
	...
	function _buildUrl(dict) {
	  ...
	  for (var _i = 0, _Object$entries = Object.entries(dict); _i < _Object$entries.length; _i++) {
	    ...
	    // æ�œç´¢name=generalè¿™ç§�å½¢å¼�çš„å­—ç¬¦ä¸²(&æ˜¯åˆ†éš”ç¬¦)
	    var pattern = "".concat(key, "=([^&]*)");
	    var targetStr = "".concat(key, "=").concat(val);
	    /*
	     * å¦‚æžœç›®æ ‡urlä¸­åŒ…å�«äº†keyé”®, æˆ‘ä»¬éœ€è¦�å°†å®ƒæ›¿æ�¢æˆ�æˆ‘ä»¬è‡ªå·±çš„val
	     * ä¸�ç„¶å°±ç›´æŽ¥æ·»åŠ å¥½äº†.
	     */
	  ...
	  _createClass(_class, [{
	    ...
	    value: function init() {
	      ...
	      if (this.options.pagination && this.options.sidePagination === 'server' && this.options.addrbar) {
	        // æ ‡å¿—ä½�, åˆ�å§‹åŠ è½½å�Žå…³é—­
	        this.addrbarInit = true;

	        var _prefix = this.options.addrPrefix || ''; // ä¼˜å…ˆçº§æŽ’åº�: ç”¨æˆ·æŒ‡å®šå€¼æœ€ä¼˜å…ˆ, æœªæŒ‡å®šæ—¶ä»Žåœ°å�€æ �èŽ·å�–, æœªèŽ·å�–åˆ°æ—¶é‡‡ç”¨é»˜è®¤å€¼
	        ...
	        this.options.onLoadSuccess = function (data) {
	          if (_this.addrbarInit) {
	            ...
	            var params = {};
	            params["".concat(_prefix, "page")] = _this.options.pageNumber, params["".concat(_prefix, "size")] = _this.options.pageSize, params["".concat(_prefix, "order")] = _this.options.sortOrder, params["".concat(_prefix, "sort")] = _this.options.sortName, params["".concat(_prefix, "search")] = _this.options.searchText; // h5æ��ä¾›çš„ä¿®æ”¹æµ�è§ˆå™¨åœ°å�€æ �çš„æ–¹æ³•
