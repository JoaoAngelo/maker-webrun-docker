...
Mask.prototype.isAllowKeyPress = function(e, o) {
  ...
  var xe = new qEvent(e);
  // O IE nÃ£o estava permitindo alterar o valor quando todo o texto havia sido selecionado e o tamanho mÃ¡ximo
  // de ediÃ§Ã£o tinha sido atingido
...
Mask.prototype.setGeneric = function(_v, _d) {
  ...
  rx = { "x": "A-Za-z", "U": "A-ZÃ€-Ãša-zÃ -Ãº", "S": "A-ZÃ€-Ãša-zÃ -Ãº0-9", "#": "0-9", "*": "A-Za-z0-9", "l": "A-ZÃ€-Ãša-zÃ -Ãº" };
  var ry = { "x": "A-Za-zÃ€-Ãº", "*": "A-Za-zÃ€-Ãº0-9", "c": " .,;:%()'{}|?&<>!{}*^_" };
...
Mask.prototype.setNumber = function(_v, _d) {
  ...
  if (!/^[\$]?((\$?[\+-]?([0#]{1,3},)?[0#]*(\.[0#]*)?)|([\+-]?\([\+-]?([0#]{1,3},)?[0#]*(\.[0#]*)?\)))$/.test(m))
    return this.throwError(1, "Uma mÃ¡scara invÃ¡lida foi definida pelo construtor.", _v);
  ...
  // Meio para contornar o fato de o objeto Number nÃ£o aceitar -0.
  ...
  if (!onlyZeros) {
    ...
    if (isNaN(vn)) return this.throwError(2, "O valor escolhido nÃ£o Ã© um nÃºmero.", "");
...
Mask.prototype.setDate = function(_v) {
  ...
  if (d.getDate() != dd) return this.throwError(1, "Um dia invÃ¡lido foi escolhido.", _v);
  else if (d.getMonth() != mm) return this.throwError(2, "Um mÃªs invÃ¡lido foi escolhido.", _v);
...
function qEvent(e) {
  ...
  // Para o IE estava entrando nessa condiÃ§Ã£o e nÃ£o podia
...
//Novo formato de telefones das regiÃµes brasileiras.
//Ex: (XX) XXXXX-XXXX
...
function verifySubstrIP(vr) {
  ...
  for (i = 0; i < vr.length; i++) {
    if (vr.substr(i, 1) == ".") {
      if (i == 0) { //nÃ£o iniciar com ponto
        vr = vr.substr(1, vr.length);
        verify = true;
        break;
      }
      if (i > 0) {
        if (vr.substr(i - 1, 1) == ".") { // nÃ£o pode existir segmento sem valor, como x..y (2 '.' juntos)
    ...
    if (qtdChr > 3) {
      if (qtdPonto < 3) { //separa apÃ³s o terceiro caracter com um '.'
        vr = vr.substr(0, i) + "." + vr.substr(i, vr.length);
        verify = true;
        break;
      } else { //jÃ¡ possui o nÃºmero mÃ¡ximo de pontos, ignora o que estiver apÃ³s o terceiro caracter
        vr = vr.substr(0, i);
        qtdChr = 3;
    ...
    if (qtdChr == 3) {
      if (vrSegment > 255) { // Segmento nÃ£o pode ser superior a 255
        vr = vr.substr(0, i - 2); //Elimina o que estiver a partir do segmento invÃ¡lido.
        break;
      }
    }
